...
export default function LearningPathPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      try {
        ...
        if (pathRes.status === 401) { removeToken(); router.push("/login"); return; }
        if (!pathRes.ok) throw new Error("Failed to fetch learning path");
        setData(await pathRes.json());
  ...
  const handleTakeTest = async (phaseIndex: number) => {
    ...
    try {
      ...
      if (!testData.questions?.length) throw new Error("No test questions received");
  ...
  const handleSubmitTest = async (answers: number[]) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/submit-test`, {
        ...
        body: JSON.stringify({ phase_index: currentTestPhase, answers }),
      });
  ...
  const getPhaseProgress = (index: number) => {
    const found = progress.find((p) => p.phase_index === index);
    if (!found && progress.length === 0 && index === 0) {
  ...
  return (
    <div className="min-h-screen bg-background text-text-main font-sans selection:bg-primary selection:text-white">
      ...
      {/* ── Hero + Stats: contained layout ── */}
      <div className="max-w-7xl mx-auto px-4 lg:px-8 pt-8 pb-6 space-y-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-surface-1 border border-border rounded-2xl p-6 flex flex-col justify-between relative overflow-hidden">
            ...
            <div>
              <h3 className="text-sm font-semibold text-text-dim uppercase tracking-wider mb-1">
                Estimated Velocity
              </h3>
              <div className="text-3xl font-bold text-text-main">
                {data.meta.duration_months} Months
              </div>
              <p className="text-xs text-text-muted mt-2">
                At {data.meta.weekly_time_hours} hours/week pace
              </p>
            </div>
      ...
      {/* ── SNAPSHOT VIEW: full-bleed, no container constraints ── */}
      {viewMode === "snapshot" && (
      ...
      {viewMode === "detailed" && (
        <div className="max-w-7xl mx-auto px-4 lg:px-8 pb-16">
                ...
                <div key={index} className={`relative pl-8 md:pl-12 ${isLocked ? "opacity-60" : ""}`}>
                  <div
                    className={`absolute -left-[9px] top-0 h-4 w-4 rounded-full border-4 border-background ${
                      isCompleted
                        ? "bg-success shadow-[0_0_0_4px_rgba(34,197,94,0.2)]"
                        : !isLocked
                        ? "bg-primary shadow-[0_0_0_4px_rgba(124,58,237,0.2)]"
                        : "bg-surface-2"
                    }`}
                  />
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 relative z-10">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 flex-wrap mb-2">
                          ...
                          {isLocked && (
                            <span className="px-2 py-1 rounded text-xs font-bold bg-surface-2 text-text-dim flex items-center gap-1 flex-shrink-0">
                              <span className="material-symbols-outlined text-sm">lock</span>LOCKED
                            </span>
                          )}
                          {isCompleted && (
                            <span className="px-2 py-1 rounded text-xs font-bold bg-success/10 text-success flex items-center gap-1 flex-shrink-0">
                              <span className="material-symbols-outlined text-sm">check_circle</span>COMPLETED
                            </span>
                          )}
                          {!isLocked && !isCompleted && (
                            <span className="px-2 py-1 rounded text-xs font-bold bg-primary text-white flex-shrink-0">
                              IN PROGRESS
                            </span>
                          )}
                        </div>
                        <div className="flex flex-wrap gap-4 text-sm text-text-muted">
                          ...
                          {phaseProgress.best_score > 0 && (
                            <div className="flex items-center gap-2">
                              ...
                              Best Score: {phaseProgress.best_score}%
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      {!isLocked && (
                        ...
                        >
                          ...
                          <span className="text-black">
                            {testLoading ? "Loading..." : isCompleted ? "Retake Test" : "Take Test"}
                          </span>
                        </button>
                      )}
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {phase.resources?.map((resource: any, rIndex: number) => (
                          <a key={rIndex} href={resource.link} target="_blank" rel="noopener noreferrer"
                            className="group flex flex-col p-4 bg-surface-1 border border-border rounded-xl hover:border-primary/50 transition-all"
                          >
                            ...
                            <h4 className="text-sm font-bold text-text-main mb-1 line-clamp-2 group-hover:text-primary transition-colors">
                              {resource.title}
                            </h4>
          ...
          {/* Completion Banner */}
          {allPhasesCompleted && (
            <div className="relative overflow-hidden bg-gradient-to-br from-success/10 via-primary/5 to-success/10 border-2 border-success/30 rounded-2xl p-8 md:p-12 text-center mt-8">
              <div className="relative z-10 space-y-4">
                ...
                <p className="text-lg text-text-muted max-w-xl mx-auto">
                  Congratulations! You've successfully completed all {totalPhases} phases. You've demonstrated mastery across every topic.
                </p>
                <div className="flex flex-wrap justify-center gap-4 pt-4">
                  <Link href="/profile" className="px-6 py-3 bg-primary hover:bg-primary-hover text-white rounded-xl font-bold transition-all flex items-center gap-2 shadow-lg hover:shadow-xl">
                    <span className="material-symbols-outlined">person</span>View Profile
                  </Link>
                  <Link href="/market-insights" className="px-6 py-3 bg-surface-2 hover:bg-surface-3 text-text-main rounded-xl font-bold transition-all flex items-center gap-2 border border-border">
      ...
      {showResultModal && testResult && (
        <TestResultModal
          ...
          onClose={() => { setShowResultModal(false); setTestResult(null); }}
